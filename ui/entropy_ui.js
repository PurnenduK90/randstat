/**
 * EntropyAnalyzer UI Component Library (WASM)
 * Embeddable randomness test suite & Chi-Square visualization component with Alpha selector, Guardrails, and Markdown/JSON Exporter.
 */
...
class EntropyAnalyzer {
   ...
   * @param {string} [options.wasmUrl='entropy.wasm'] - Path to WASM binary
  ...
  constructor(container, options = {}) {
    ...
    this.options = {
      wasmUrl: options.wasmUrl || 'entropy.wasm',
      theme: options.theme || {},
      df: 255
    };
  ...
  async loadWasm() {
    ...
    try {
      ...
      if (typeof EntWasmRunner !== 'undefined') {
  ...
  async processFile(file) {
    ...
    let rawChip = 0.5;
    let exceedText = "N/A";
    if (this.wasmInstance.exports.chi2_pochisq) {
  ...
  downloadReport(format = 'markdown') {
    ...
    const critFn = this.wasmInstance.exports.chi2_critical_value || this.wasmInstance.exports.chi2_critical_value_export;
...
if (typeof customElements !== 'undefined' && !customElements.get('entropy-analyzer')) {
  customElements.define('entropy-analyzer', class extends HTMLElement {
    connectedCallback() {
      const wasmUrl = this.getAttribute('wasm-url') || 'entropy.wasm';
