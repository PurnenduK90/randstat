//! Zero-dependency `no_std` SHA-256 streaming hash accumulator.
//!
...
impl Sha256 {
    /// Creates a new, zero-initialised `Sha256` accumulator.
    #[inline]
    ...
    /// Feeds bytes into the hash accumulator.
    ...
    pub fn finalize(&self) -> [u8; 32] {
        ...
        for i in c.buf_len..56 {
            c.buffer[i] = 0;
        }
        c.buffer[56..64].copy_from_slice(&total_bits.to_be_bytes());
        let block = c.buffer;
        c.transform(&block);

        let mut out = [0u8; 32];
        for i in 0..8 {
            out[i * 4..(i + 1) * 4].copy_from_slice(&c.state[i].to_be_bytes());
        }
        out
    }
